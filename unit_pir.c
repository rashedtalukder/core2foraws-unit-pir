/*!
 * @brief Library for the PIR motion sensor unit on Port B of the Core2 for AWS
 * IoT Kit
...
esp_err_t unit_pir_init( void )
{
  ...
  esp_err_t err =
      core2foraws_expports_digital_read( PIR_SENSOR_PIN, &test_reading );
  if( err != ESP_OK )
  {
    ESP_LOGE( TAG, "Failed to read from PIR sensor GPIO: %s",
              esp_err_to_name( err ) );
...
esp_err_t unit_pir_read( bool *motion_detected )
{
  ...
  esp_err_t err =
      core2foraws_expports_digital_read( PIR_SENSOR_PIN, motion_detected );
  if( err != ESP_OK )
  {
...
esp_err_t unit_pir_deinit( void )
{
  ...
  if( err != ESP_OK )
  {
    ESP_LOGW( TAG, "Failed to reset PIR sensor GPIO: %s",
              esp_err_to_name( err ) );
  }
